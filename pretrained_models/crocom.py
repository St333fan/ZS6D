...
class CroCoNet(nn.Module):
    ...
    def _encode_image(self, image, do_mask=False, custom_mask=None, return_all_blocks=False):
        """
    ...
    def forward(self, img1, img2, custom_mask=None):
        """
        img1: tensor of size B x 3 x img_size x img_size
        img2: tensor of size B x 3 x img_size x img_size
        custom_mask: optional custom mask of size B x N (where N is the number of patches)
        
        out will be    B x N x (3*patch_size*patch_size)
        masks are also returned as B x N just in case 
        """
        # encoder of the masked first image 
        feat1, pos1, mask1 = self._encode_image(img1, do_mask=True, custom_mask=custom_mask)
